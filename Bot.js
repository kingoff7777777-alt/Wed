...
var SYS = {
vi: “Ban la AI tro ly cua iprights.asia - trang ca nhan cua 부이뒤칸 (2010, Viet Nam) va 문준우 (2011, Han Quoc). Tra loi tieng Viet, than thien, ngan gon. Web co: Album anh, Chat, Profile.”,
kr: “당신은 iprights.asia의 AI 어시스턴트입니다. 부이뒤칸(2010, 베트남)과 문준우(2011, 한국)의 개인 사이트입니다. 한국어로 친근하게 답변하세요.”,
en: “You are the AI assistant of iprights.asia - site of 부이뒤칸 (2010, Vietnam) and 문준우 (2011, Korea). Reply in English, friendly and brief.”
};
var GREET = {
...
var ERR  = { vi: “Co loi xay ra, thu lai nhe!”, kr: “오류가 발생했습니다!”, en: “An error occurred!” };
var SUGS = {
vi: [“Web nay co gi?”, “부이뒤칸 la ai?”, “Dang bai the nao?”, “Chat o dau?”],
...
css.textContent =
“#ipr-fab{position:fixed;bottom:22px;right:16px;z-index:99990;” +
“width:52px;height:52px;border-radius:50%;” +
“background:linear-gradient(135deg,#a855f7,#38bdf8);” +
“border:none;cursor:pointer;font-size:22px;” +
“display:flex;align-items:center;justify-content:center;” +
“box-shadow:0 4px 20px rgba(168,85,247,.6);” +
“animation:iprPulse 2.5s infinite;transition:transform .2s;}” +
“#ipr-fab:hover{transform:scale(1.08);}” +
“@keyframes iprPulse{0%,100%{box-shadow:0 4px 20px rgba(168,85,247,.5)}” +
“50%{box-shadow:0 4px 32px rgba(56,189,248,.8)}}” +
“#ipr-win{position:fixed;bottom:82px;right:12px;z-index:99989;” +
...
“#ipr-online{font-family:Nunito,sans-serif;font-size:10px;color:#34d399;}” +
“#ipr-x{margin-left:auto;background:none;border:none;” +
“color:rgba(240,234,255,.5);font-size:18px;cursor:pointer;padding:2px 4px;}” +
...
“.ipr-av{width:24px;height:24px;border-radius:50%;” +
“background:linear-gradient(135deg,#a855f7,#38bdf8);” +
“display:flex;align-items:center;justify-content:center;” +
“font-size:11px;flex-shrink:0;}” +
“.ipr-av.me{background:linear-gradient(135deg,#ff6eb4,#a855f7);}” +
...
“#ipr-inp-wrap{padding:8px 10px;background:#0f0a1e;” +
...
“#ipr-send:hover{transform:scale(1.08);}” +
“#ipr-send:disabled{opacity:.5;cursor:not-allowed;transform:none;}” +
...
“<div id="ipr-hd">” +
“<div id="ipr-av">🤖</div>” +
“<div><div id="ipr-name">AI Assistant</div><div id="ipr-online">● Online</div></div>” +
...
if (isOpen) { inp.placeholder = PH[lang()] || PH.vi; inp.focus(); }
}
...
av.className = "ipr-av" + (role === "user" ? " me" : "");
...
var L = lang();
var list = SUGS[L] || SUGS.vi;
list.forEach(function (s) {
var btn = document.createElement(“button”);
btn.className = “ipr-sug”;
btn.textContent = s;
btn.addEventListener(“click”, function () { inp.value = s; doSend(); });
...
var bubble = addMsg(
  L === "kr" ? "생각 중..." : L === "en" ? "Thinking..." : "Dang suy nghi...",
  "ai", true
);

var contents = [];
if (history.length === 1) {
  contents.push({ role: "user", parts: [{ text: (SYS[L] || SYS.vi) + "\n\n" + text }] });
} else {
  contents.push({ role: "user", parts: [{ text: SYS[L] || SYS.vi }] });
  history.slice(-8).forEach(function (m) { contents.push(m); });
}
...
fetch(URL, {
  ...
  .then(function (d) {
    var reply = (d.candidates && d.candidates[0] &&
      d.candidates[0].content && d.candidates[0].content.parts &&
      d.candidates[0].content.parts[0] &&
      d.candidates[0].content.parts[0].text) || (ERR[lang()] || ERR.vi);
    bubble.textContent = reply;
    bubble.classList.remove("think");
    history.push({ role: "model", parts: [{ text: reply }] });
  })
  .catch(function () {
    bubble.textContent = ERR[lang()] || ERR.vi;
    bubble.classList.remove("think");
  })
  .finally(function () {
    send.disabled = false;
...
/* ── Init ─────────────────────────────────────────── */
addMsg(GREET[lang()] || GREET.vi, “ai”, false);
renderSugs();
}());
